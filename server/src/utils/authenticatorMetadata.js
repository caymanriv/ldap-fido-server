/**
 * Known hardware authenticator AAGUIDs and their corresponding device names
 * Source: https://webauthn.passwordless.id/demos/authenticators
 */

// Main authenticator metadata mapping AAGUIDs to human-readable names
const AUTHENTICATOR_METADATA = {
    ...
    '3fd410dc-8ab7-4b86-a1cb-c7174620b2dc': 'IDEMIA SOLVO Fly 80 R1 FIDO Card',
    ...
    '664d9f67-84a2-412a-9ff7-b4f7d8ee6d05': 'OpenSK hardware authenticator',
    ...
    '8652abe9-fbd8-4810-a840-d6fc442a8c2c': 'SoloKeys Solo 2 Security Key NFC+USB-A',
    ...
    'e1a96183-5016-4f24-b55b-e3ae23614cc6': 'ATKey.ProS CTAP2.0',
    'e400ef8c-711d-4692-af46-7f2cf7da23ad': 'Swissbit iShield Key 2 Enterprise',
    'e416201b-afeb-41ca-a03d-2281c28322aa': 'ATKey.ProS CTAP2.1',
...
/**
 * Get the device name for a given AAGUID
 * @param {string} aaguid - The AAGUID to look up (with or without hyphens)
 * @returns {string} The device name or null if not found
 */
function getDeviceName(aaguid) {
  if (!aaguid) return null;
  
  // Convert to lowercase and remove any hyphens for consistent comparison
  const cleanAaguid = aaguid.toLowerCase().replace(/-/g, '');
  
  // Try to find a direct match first (for backward compatibility)
  if (AUTHENTICATOR_METADATA[aaguid.toLowerCase()]) {
    return AUTHENTICATOR_METADATA[aaguid.toLowerCase()];
  }
  
  // Search through all keys for a match without hyphens
  for (const [key, value] of Object.entries(AUTHENTICATOR_METADATA)) {
    if (key.toLowerCase().replace(/-/g, '') === cleanAaguid) {
      return value;
    }
  }
